...
router.get("/status_mbr_assy/:mc/:date", async (req, res) => {
    ...
    let result = await MBR_table.sequelize.query(
      `SELECT *
      FROM (
        SELECT registered_at AS start_time,
          occurred AS end_time, mc_status, mc_no,
          ROW_NUMBER() OVER (PARTITION BY occurred ORDER BY registered_at) AS row_num
        FROM 
          [data_machine_assy].[dbo].[DATA_MCSTATUS_ASSY]
...
//status MBR
router.get("/status_mc_mbr", async (req, res) => {
  let { mc } = req.params;
  let { date } = req.params;
  try {
  let result = await MBR_table.sequelize.query(
  ` SELECT top 10 occurred, mc_status, mc_no, process,
  ROW_NUMBER() OVER (PARTITION BY mc_no ORDER BY occurred) AS rn,
ROW_NUMBER() OVER (PARTITION BY occurred ORDER BY registered_at) AS num
FROM [data_machine_assy].[dbo].[DATA_MCSTATUS_ASSY]
where format (iif(DATEPART(HOUR, [occurred])<8,dateadd(day,-1,[occurred]),[occurred]),'yyyy-MM-dd') = '2024-10-07'
and mc_no = 'mbrma02'
